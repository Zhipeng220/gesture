...
class HGCNLayer(nn.Module):
    ...
    def forward(self, x, bone_features):
        B, N, C = x.shape;
        ...
        adj = F.softmax((q @ k.transpose(-2, -1)) / (self.head_dim ** 0.5), dim=-1)
        out = (adj @ v).transpose(1, 2).reshape(B, N, C);
        return self.out_proj(self.dropout(out)), adj


class GNNBlock(nn.Module):
    # MODIFIED: Tier 1-B - Added use_residual flag
    def __init__(self, dim, num_heads, num_gnn_layers=2, gnn_type='HGCN', dropout=0.1, use_residual=True, **kwargs):
        super().__init__()
    ...
    def forward(self, x, bone_features=None):
        ...
        for norm, layer in self.layers:
            h = norm(x);
            h_out, attn = layer(h, bone_features);
            all_attn_weights.append(attn);
            # MODIFIED: Tier 1-B - Make residual connection configurable
...
class SpatioTemporalGraphBlock(nn.Module):
    # MODIFIED: Tier 1-B - Pass gnn_residual to GNNBlock
...
class TCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, dilations=[1, 2, 4], dropout=0.2):
        ...
        for i in range(num_layers):
            ...
            # CORRECTED: Replaced manual padding calculation with padding='same'.
            # This ensures the output sequence length is the same as the input.
            conv = nn.Conv1d(
    ...
    def forward(self, x):
        # Input x shape: (B, T, C) -> permute to (B, C, T) for Conv1d
        out = x.permute(0, 2, 1)
...
class TemporalAttentionBlock_V4(nn.Module):
    # MODIFIED: Tier 1-C (attention_dropout), Tier 2-D (use_temporal_conv)
...
# NEW: Tier 2-F - Simple policy network for adaptive depth
...
class DHSGNet_V4(nn.Module):
    def __init__(self, input_dim, num_classes, seq_len=32, num_layers=8, num_heads=12, embed_dim=512, dropout=0.2,
        ...
        # MODIFIED: Pass all kwargs to the blocks to handle new parameters
        for i in range(num_layers):
    ...
    def forward(self, x, skeletal_features=None, key_padding_mask=None, labels=None) -> Dict[
        ...
        # NEW: Tier 2-F - Adaptive Depth Logic
        layer_scores = None
        if self.adaptive_depth and self.training:
            # Policy uses the mean of initial features to decide which layers to use
        ...
        for i, (block, drop_path) in enumerate(self.blocks):
            # NEW: Tier 2-F - Conditionally skip a layer during training
            if self.adaptive_depth and self.training:
