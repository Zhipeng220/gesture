...
def time_scaling(sequence: np.ndarray, scale_factor: float) -> np.ndarray:
    ...
    for i in range(N):
        for j in range(D):
            ...
            resampled_sequence[:, i, j] = interp_func(np.linspace(0, T - 1, T))
    return resampled_sequence


def mixup_data(x, y, alpha=1.0, use_skeletal=False):
    lam = np.random.beta(alpha, alpha) if alpha > 0 else 1.0
    index = torch.randperm(x.size(0)).to(x.device)
    mixed_x = lam * x + (1 - lam) * x[index, :]
    if use_skeletal: return mixed_x, y, y[index], lam
...
# CORRECTED: Re-inserted the missing function definition
...
# NEW: Data & Training Improvements - Spatial Noise augmentation function
...
# NEW: Data & Training Improvements - Temporal Jittering augmentation function
def apply_temporal_jitter(landmarks, drop_rate=0.1):
    """Randomly drops a percentage of frames and pads back to original length."""
    if np.random.rand() < 0.5:  # Apply jitter with 50% probability
        return landmarks
    ...
    if current_len == 0:  # Handle case where all frames are dropped
        return torch.zeros_like(landmarks)

    if current_len < original_len:
        pad_len = original_len - current_len
        padding = jittered[-1:, ...].repeat(pad_len, 1, 1)  # Pad with the last valid frame
...
class AdvancedGestureDataset(StandardGestureDataset):
    ...
    def collate_fn(batch, mixup_alpha=0.4):
        ...
        if sequences.size(0) > 1 and np.random.rand() < 0.5 and mixup_alpha > 0:
            mixed_seqs, y_a, y_b, lam = mixup_data(sequences, labels, alpha=mixup_alpha)
            if use_skeletal:
                mixed_skeletal, _, _, _ = mixup_data(skeletal_seqs, labels, alpha=mixup_alpha)
                return mixed_seqs, mixed_skeletal, y_a, y_b, torch.tensor(lam, dtype=torch.float32), masks
            return mixed_seqs, y_a, y_b, torch.tensor(lam, dtype=torch.float32), masks

        if use_skeletal:
            return sequences, skeletal_seqs, labels, labels, torch.tensor(1.0, dtype=torch.float32), masks
        return sequences, labels, labels, torch.tensor(1.0, dtype=torch.float32), masks
...
def plot_confusion_matrices(model, test_ds, le, device, save_path, use_arcface=False):
    model.eval()
